...
SAVE_DIR = current_directory + "/../model/test_attention"
episode_start = 1
...
class SelfAttentionObstacle(nn.Module):
    ...
    def forward(self, obs, mask=None):
        ...
        if mask is not None:
            ...
            sum_feat = (attn_output * mask_inv).sum(dim=1)  # sum only valid features
            count = mask_inv.sum(dim=1).clamp(min=1.0)  # avoid division by 0
            return sum_feat / count  # [batch, hidden_dim]
        else:
            return attn_output.mean(dim=1)  # [batch, hidden_dim]
