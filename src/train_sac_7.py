...
from utils.sac_dataset3   import SACDataset
...
# Other
visulaize_step = 20
MAX_STEP = 300         # maximun available step per episode
current_file_path = os.path.abspath(__file__)
current_directory = os.path.dirname(current_file_path)
SAVE_DIR = current_directory + "/../model/SAC_linear_7"
...
if use_data:
    for data in sim.load_data():
        s1, s2, a, r, sn1, sn2 = data
        memory.add_dataset(
            torch.tensor(s1, dtype=torch.float32, device=device).unsqueeze(0),
            torch.tensor(s2, dtype=torch.float32, device=device),
            torch.tensor(a, dtype=torch.float32, device=device).unsqueeze(0),
            torch.tensor(r, dtype=torch.float32, device=device).unsqueeze(0).unsqueeze(0),
            torch.tensor(sn1, dtype=torch.float32, device=device).unsqueeze(0),
            torch.tensor(sn2, dtype=torch.float32, device=device),
        )
